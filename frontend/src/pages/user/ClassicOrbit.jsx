...
export default function ClassicOrbit() {
    ...
    React.useEffect(() => {
        ...
        TransferService.onTransferRequest = (request) => {
            ...
            // Show notification dialog instead of auto-accepting
            setIncomingTransfer({
                transferId: request.transferId,
                fileName: request.fileName,
                filename: request.fileName, // For dialog display
                size: request.size,
                senderId: request.senderId,
                sender: request.senderId || 'Unknown Device',
                fileType: request.mimeType || 'application/octet-stream',
                downloadUrl: null // Not used for direct transfer
            });
    ...
    const handleAcceptTransfer = async () => {
        ...
        try {
            ...
            if ('showSaveFilePicker' in window) {
                try {
                    fileHandle = await window.showSaveFilePicker({
                        suggestedName: incomingTransfer.fileName || incomingTransfer.filename,
                        types: [{
                            description: 'File',
                            accept: {
                                [incomingTransfer.fileType || 'application/octet-stream']: [(incomingTransfer.fileName || incomingTransfer.filename).split('.').pop() || '']
                            }
                        }]
                    });
                } catch (err) {
            ...
            toast.success(`Accepting transfer: ${incomingTransfer.fileName || incomingTransfer.filename}...`);
            ...
            {!isPro && (
                <div className="mb-4 rounded-2xl border border-amber-200 dark:border-amber-500/30 bg-amber-50 dark:bg-amber-500/10 p-4 flex items-center justify-between gap-4">
                    <div className="text-sm text-amber-800 dark:text-amber-200">
                        Free plan: 3 transfers limit. Upgrade to Pro for unlimited transfers.
                    </div>
                    <a
                        href="/pricing"
